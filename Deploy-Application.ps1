...
Try {
	...
	[string]$appVendor = 'Pingman Tools'
	[string]$appName = 'PingPlotter'
	[string]$appVersion = '5.23.3.8770'
	[string]$appArch = 'x64'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '2022.09.02'
	...
	$CloseApps = "PingPlotter"

	##* Application install parameters.
	# 64-bit application
	# Application install Path.
	$64bitAppInstallPath = "$PSScriptRoot\Files\pingplotter_install.exe"
	# Application  install parameters.
	$64bitAppInstallParam = "/quiet" 
	...
	#$RemoveAppNamesMSI = @("")
	$RemoveAppNamesEXE = @("PingPlotter")

	##* Application uninstall parameters.
	# 64-bit application
	# Application uninstall path.
	$64bitAppUninstallPath = "C:\Windows\System32\MsiExec.exe"
	# Application uninstall parameters.
	$64bitAppUninstallParam = "/x {2744D183-39DE-4473-A4FD-B6D0961D0A12} /qn"
	$64bitAppUninstallParam2 = "/x {10361C0B-6D3A-42C8-A6A7-F84BCC2949CB} /qn"
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Foreach ($RemoveAppNameEXE in $RemoveAppNamesEXE) {
			...
			If ($null -ne $RemoveAppNamesEXECheck) {
				If (Test-Path -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{2744D183-39DE-4473-A4FD-B6D0961D0A12}") {
				...
				If (Test-Path -Path "HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\{2744D183-39DE-4473-A4FD-B6D0961D0A12}") {
					...
					# Uninstall previous EXE versions of application(s).      
					Try {
						Execute-Process -FilePath "$64bitAppUninstallPath" -Arguments "$64bitAppUninstallParam2"
					}
					# If uninstall failed, log results. Exit script.
					Catch [System.Exception] {
						Write-Log -Message "Uninstalling previous EXE versions of $RemoveAppNameEXE failed with error: $_."
